 ...
 * 所有其他模块通过 `import { CONFIG, THEMES } from './config.js'` 使用。
 *
 * 版本历史：
 *   - v9.0.0：模块化重构
 *   - v9.0.1：版本号与 index.html 的 <title> 保持一致
 *   - v9.0.3：随全项目 bug 修复同步更新版本号
 *             （修复 auth.js 的 applyRateLimit 缺失、
 *               batch.js 批量操作后未隐藏批量栏、
 *               import-export.js 覆盖模式导入统计错误）
 *   - v9.0.4：移动端默认收起顶部工具栏
 *             （views.js 在 renderApp() 中依据窗口宽度决定是否
 *               初始加上 top-collapsed 类；本文件仅同步版本号）
 *   - v9.0.5：修复 CSP（恢复 form-action 'none'）、
 *             handleResize 用 CONFIG.VIRTUAL_MOBILE_BREAKPOINT 替代硬编码 640、
 *             import-export.js 导入日志用 actualAddedCount。
 *             本文件同步更新 APP_VERSION 至 9.0.5。
 */
...
export const THEMES = Object.freeze({
    ocean: {
        ...
        vars: {
            ...
            '--cat-count-bg': 'rgba(0, 0, 0, 0.08)',
            '--cat-count-color': '#334155',
            '--warning-box-bg': '#fff8eb',
            '--warning-box-color': '#8c4f00',
            '--highlight-bg': '#ffe484',
    ...
    sunset: {
        ...
        vars: {
            ...
            '--primary-light': 'rgba(234,88,12,0.2)',
            '--danger': '#b91c1c',
            '--danger-hover': '#991b1b',
            '--warning': '#b45309',
            '--success': '#059669',
    ...
    sakura: {
        ...
        vars: {
            ...
            '--primary-light': 'rgba(219,39,119,0.2)',
            '--danger': '#b91c1c',
            '--danger-hover': '#991b1b',
            '--warning': '#b45309',
            '--success': '#059669',
    ...
    midnight: {
        ...
        vars: {
            ...
            '--btn-bg-active': 'rgba(255, 255, 255, 0.08)',
            '--cat-count-bg': 'rgba(255, 255, 255, 0.18)',
            '--cat-count-color': '#e2e8f0',
            '--warning-box-bg': '#2f2412',
            '--warning-box-color': '#fadf8a',
            '--highlight-bg': '#4a3a0a',
